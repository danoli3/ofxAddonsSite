...
<div class="admin-tabs">
  <?php foreach ([...OFX_ADMIN_TYPES, OFX_ADMIN_CURATED_TAB, OFX_ADMIN_AI_CURATED_TAB, OFX_ADMIN_NO_DESC_TAB, 'Deleted'] as $t): ?>
    <?php $tabLabel = $t === OFX_ADMIN_NO_DESC_TAB ? 'No Description' : ($t === OFX_ADMIN_AI_CURATED_TAB ? 'AI Curated' : ofx_h($t)); ?>
    <a href="/admin/repos?type=<?= ofx_h($t) ?>&sort=<?= ofx_h($sort) ?><?= $qSuffix ?>" class="admin-tab <?= $type === $t ? 'active' : '' ?>" data-type="<?= ofx_h($t) ?>">
      <?= $tabLabel ?> <span class="count"><?= $counts[$t] ?></span>
    </a>
  <?php endforeach; ?>
</div>
